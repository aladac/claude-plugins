...
Plugins are installed from registered marketplaces in `settings.json`:
```bash
/plugins:install <plugin-name>@<marketplace>
```

## Testing Plugin Changes

After making changes to plugin files (agents, skills, commands, hooks):

1. If using a marketplace: run `/plugins:update` to pull changes
2. **Restart Claude Code** — plugins are loaded at startup, not dynamically

| Change Type | Requires Restart |
|-------------|------------------|
| Agent/Skill/Command content | Yes |
