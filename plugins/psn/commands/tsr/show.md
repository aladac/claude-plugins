...
Single image:
```bash
bash ~/Projects/personality-plugin/skills/tsr/tsr.sh hud "$FILE" --title "IMAGE" --caption "$CAPTION"
```

Multiple images:
```bash
bash ~/Projects/personality-plugin/skills/tsr/tsr.sh grid $FILES --title "GALLERY" --columns $N
```
